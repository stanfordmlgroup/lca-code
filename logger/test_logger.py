...
class TestLogger(BaseLogger):
    ...
    def __init__(self, args, dataset_len, norm_dict):
        assert not args.is_training
        super(TestLogger, self).__init__(args, dataset_len, norm_dict)
    ...
    def end_epoch(self, metrics, counter, optimizer=None):
        """Log info for end of an epoch.

        Args:
            metrics: Dictionary of metric values. Items have format '{phase}_{metric}': value.
            counter: Counter of epochs.
            optimizer: Optimizer being used (applies to training only).
        """
