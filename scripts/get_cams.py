...
from dataset.constants import IMAGENET_MEAN, IMAGENET_STD, TCGA_MEAN, TCGA_STD
...
def save_grad_cams(args, loader, model, output_dir, only_competition=False, only_top_task=False):
    ...
    study_level = False #loader.dataset.study_level
    ...
    if study_level:
        ...
        for inputs, labels, info_dict in loader:
...
def write_grad_cams(input_, label, grad_cam,
    ...
    img = util.un_normalize_cam(input_, TCGA_MEAN, TCGA_STD)
    ...
    with torch.set_grad_enabled(True):
        ...
        for task_id in range(num_tasks):
            ...
            task = task.replace(' ', '_')
            if num_tasks == 2:
                task_label = int(label[0] == task_id)
            else:
            ...
            img_with_cam = util.add_heat_map(img, resized_cam, normalize=False, alpha_img=0.5)

            # Save a cam for this task and image
            # using task, prob and groundtruth in file name
            if len(probs) == 1:
                prob = probs[0] if task_id == 1 else 1 - probs[0]
            else:
